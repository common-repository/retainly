	...
	$( document ).ready( function() {
		...
		$body.on( 'click', '.rad_dashboard_icon_shortcode, .rad_dashboard_next_shortcode button', function() {
			...
			if ( 'locked' == shortcode_type ) {
				shortcode_text = '[rad_retainly_locked optin_id="' + optin_id + '"] content [/rad_retainly_locked]';
			} else if (click_trigger == true) {
				shortcode_text = '[retainly_on_click_intent optin_id='+ optin_id + '] [/retainly_on_click_intent]]';
			}else{
                shortcode_text = '[rad_retainly_inline optin_id="' + optin_id + '"]';
            }
